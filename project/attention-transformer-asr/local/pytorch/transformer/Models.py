...
class TransferQA(nn.Module):
    def __init__(self, encoder, n_tgt_vocab, dropout=0.1):
        super(TransferQA, self).__init__()
        self.encoder = encoder
        self.tgt_word_proj = Linear(encoder.d_model, n_tgt_vocab, bias=False)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, src):
        ...
        # 1 * batch_size * seq_len * d_model, proper size for maxpool2d
        enc_output = F.max_pool2d(enc_output, (enc_output.size(2), 1))
        # 1 * batch_size * 1 * d_model
        enc_output = enc_output.squeeze()
        # batch_size * d_model

        seq_logit = self.tgt_word_proj(enc_output)
        # batch_size * d_output
