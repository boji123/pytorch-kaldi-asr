...
if [ $stage -le 0 ]; then
    ...
    for dataset in train test; do
        ...
        python3 local/trim_instance_length.py -data_dir data/$dataset -max_len $max_len
    done
fi

if [ $stage -le 1 ]; then
    echo '[PROCEDURE] preparing vocabulary for output label'
    python3 local/prepare_vocab.py -read_instances_file data/train/text.filtered -save_vocab_file exp/vocab.torch
fi
...
if [ $stage -le 2 ]; then
    ...
    python3 local/initialize_model.py \
        -read_feats_scp_file data/train/feats.filtered.scp \
...
if [ $stage -le 3 ]; then
    ...
    $cuda_cmd train.log CUDA_VISIBLE_DEVICES=3 python3 -u local/train.py \
        -read_feats_scp_file data/train/feats.filtered.scp \
        -read_text_file data/train/text.filtered \
        -read_vocab_file exp/vocab.torch \
